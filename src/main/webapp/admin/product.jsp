...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            论文名称：<input name="pname" id="pname">&nbsp;&nbsp;&nbsp;
            论文类型：<select name="typeid" id="typeid">
            <option value="-1">请选择</option>
            <c:forEach items="${typeList}" var="pt">
                <option value="${pt.typeId}">${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            论文字数：<input name="lprice" id="lprice">-<input name="hprice" id="hprice">
    ...
    <div id="table">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <div>
                    <h2 style="width:1200px; text-align: center;color: orangered;margin-top: 100px">暂时没有符合条件的论文！</h2>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        if(cks.length == 0){
            ...
            var str = "";
            var pid = "";
            if(confirm("您确定要删除"+cks.length+"条论文吗?")){
               // alert("可以进行删除!");
                //获取其value的值,进行字符串拼接
                $.each(cks,function () {
                    pid = $(this).val();
